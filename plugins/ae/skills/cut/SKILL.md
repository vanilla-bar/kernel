...
devブランチを起点にfeatureブランチを作成し、ドラフトPRまで一気に作成する。
...
AskUserQuestionツールを使い、生成したブランチ名をユーザーに提示して確認する：
...
```bash
gh pr create --draft --base dev --title "{Issueタイトル}" --body "$(cat <<'EOF'
Closes #{番号}
EOF
)"
```
...
```bash
gh pr create --draft --base dev --title "{説明テキスト}" --body "$(cat <<'EOF'
（作業中）
EOF
)"
```
...
```
✅ ブランチ `{BRANCH_NAME}` を作成し、ドラフトPRを作成しました（origin/dev起点）
   PR: {PR_URL}
```

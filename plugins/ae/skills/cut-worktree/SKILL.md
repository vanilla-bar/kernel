...
devブランチを起点にfeatureブランチをworktreeとして作成し、ドラフトPRまで一気に作成する。
...
# worktree作成
git worktree add -b {BRANCH_NAME} ../worktrees/{BRANCH_NAME} origin/dev
...
# ドラフトPR作成
gh pr create --draft --base dev --title "{タイトル}" --body "$(cat <<'EOF'
{本文}
EOF
)"
```
...
**1項目の場合:**
```
✅ Worktree `{BRANCH_NAME}` を作成し、ドラフトPRを作成しました（origin/dev起点）
...
**2項目以上の場合:**
```
✅ {N}個のworktreeを作成しました（origin/dev起点）
